...
import { runTest } from "../lib/testExecutor.js";
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const [passedRow] = await db
      .select({ count: count() })
      .from(testRunsTable)
      .where(eq(testRunsTable.status, "passed"));
    const [failedRow] = await db
      .select({ count: count() })
      .from(testRunsTable)
      .where(eq(testRunsTable.status, "failed"));
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // Start test execution in background (do not await)
...
// GET /api/tests/:id/stream  (SSE — must come before /:id)
router.get("/:id/stream", async (req, res) => {
  ...
  // If already complete, immediately send done event with existing data
...
router.get("/:id/pdf", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="spirale-report-${id}.pdf"`
    );
    doc.pipe(res);
    ...
    doc
      .fontSize(12)
      .font("Helvetica")
      .text(`Test Run #${run.id}`, { align: "center" });
    doc.moveDown(0.3);
    ...
    for (const step of steps) {
      ...
      doc
        ...
        .text(`${icon} Step ${step.stepIndex + 1}: ${step.action}${dur}`, { continued: false });
      doc
        .fontSize(8)
        .font("Helvetica")
        .fillColor("#374151")
